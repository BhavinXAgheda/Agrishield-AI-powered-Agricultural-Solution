...
def pred_cot_dieas(cott_plant):
    ...
    if pred == 0:
        return "Healthy Cotton Plant", 'des.html' # if index 0 burned leaf
   
   
    else:
        return "Healthy Cotton Plant", 'des.html' # if index 3
...
# Function to run the server on localhost
def run_localhost():
    app.run(host='127.0.0.1', port=5000, threaded=False)

# Function to run the server for mobile access
def run_mobile():
    app.run(host='0.0.0.0', port=5001, threaded=False)

if __name__ == "__main__":
    # Start the localhost server
    threading.Thread(target=run_localhost).start()
    
    # Start the mobile access server
    threading.Thread(target=run_mobile).start()
